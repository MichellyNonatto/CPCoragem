...
class DispatchItem(OleItem):
    ...
    def MakeDispatchFuncMethod(self, entry, name, bMakeClass=1):
        ...
        # The runtime.txt translation of the return types is expensive, so when we know the
        # return type of the function, there is no need to check the type at runtime.txt.
...
def _ResolveType(typerepr, itypeinfo):
    ...
    if type(typerepr) == tuple:
        ...
        if indir_vt == pythoncom.VT_CARRAY:  # runtime.txt has no support for this yet.
